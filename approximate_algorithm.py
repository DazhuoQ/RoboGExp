...
class SubFunction(object):
    def __init__(self, graph, influence_state, diversity_state, gamma, elements):
        ...
        self.diverdsity_state = diversity_state
    ...
    def cal(self, element):
        counter = self.current_union.union(self.influence_state[element[0]])
        div_counter = self.current_div_union.union(self.diverdsity_state[element[0]])
        return float(len(counter)) / self.graph.num_nodes + self.gamma * float(len(div_counter)) / self.graph.num_nodes
        # + float(len(diverse_counter)) / self.graph.num_nodes
...
def PatternMatch(graph_set):
    ...
    edge_x_sub_set = copy.deepcopy(origin_x_sub_set)
    edge_loss = EdgeLoss(edge_x_sub_set, pattern_set)
    ...
    for pattern in pattern_set:
        ...
        pattern_edge_num = pattern_edge_num + pattern.number_of_edges()
    x_sub_node_num = 0
    x_sub_edge_num = 0
    for x_sub in origin_x_sub_set:
        x_sub_node_num = x_sub_node_num + x_sub.number_of_nodes()
        x_sub_edge_num = x_sub_edge_num + x_sub.number_of_edges()
    x_sub_remaining = 0
    ...
    print(f'# of edges of all patterns: {pattern_edge_num}')
    print(f'# of nodes of all ex_sub: {x_sub_node_num}')
    print(f'# of edges of all ex_sub: {x_sub_edge_num}')
    print(f'# of remaining disconnected nodes in ex_sub: {x_sub_remaining}')


def visual_pattern(pattern_set, dataset_name):
    plot_util = PlotUtils(dataset_name)
    counter = 0
    for graph in pattern_set:
        print(graph)
        max_id = 0
        ...
        plot_util.plot(graph, x=x, fig_name='pattern_' + str(counter))
        counter += 1
...
def evaluation(graph, solution, model, num_classes, device, dataset_name, graph_id_begin):
    ...
    for element in solution:
        ...
        subset, _, _, _ = k_hop_subgraph(element[0], 4, graph.edge_index)
    ...
    pattern_candidate = PatternMatch(graph_set)
    # visual_pattern(pattern_candidate, dataset_name=dataset_name)
    return _, _, _
...
def data_record(dataset_name, fidelity_minus_score, fidelity_plus_score, sparsity_score, cost_time):
    ...
    data_name = os.path.join(exp_name, f'approximate_{dataset_name}')
...
def main(config):
    ...
    graph.to('cpu')
    print("Start greedy node selection process")
    logger.debug("Start greedy node selection process")
    ...
    print("Finish greedy node selection process")
    print("Start evaluation process")
    logger.debug("Finish greedy node selection process")
    logger.debug("Start evaluation process")
    fidelity_minus_score, fidelity_plus_score, sparsity_score = evaluation(graph, result_elements, 
                                model, num_classes, device='cpu',
                                dataset_name=config.datasets.dataset_name, graph_id_begin=graph_id_begin)
    print("End evaluation process")
    logger.info("End evaluation process")
    # data_record(config.datasets.dataset_name, fidelity_minus_score, fidelity_plus_score, sparsity_score, cost_time)
